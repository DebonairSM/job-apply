...
export function ChipList({ items, variant, className = '' }: ChipListProps) {
  if (!items || items.length === 0) {
    return (
      <div className={`p-3 rounded-lg border ${variantStyles[variant].container} ${className}`}>
    ...
    <div className={`p-3 rounded-lg border ${variantStyles[variant].container} ${className}`}>
      <div className="flex flex-wrap gap-2">
        {items.map((item, index) => (
          <div
            key={index}
            className={`px-3 py-1 rounded-full text-sm font-medium border transition-all duration-200 hover:scale-105 hover:shadow-sm ${variantStyles[variant].chip}`}
            title={item}
          >
