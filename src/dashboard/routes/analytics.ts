...
// GET /api/analytics/learning - Rejection learning statistics
router.get('/learning', (req, res) => {
  try {
    const database = getDb();
    
    // Get rejection statistics
    const rejectionStats = database.prepare(`
      SELECT COUNT(*) as totalRejections FROM jobs WHERE status = 'rejected'
    `).get() as { totalRejections: number };
    
    // Get top rejection patterns
    ...
    const recentAdjustments = database.prepare(`
      SELECT profile_category, old_weight, new_weight, reason, created_at
    ...
    // Get current weight adjustments summary
    const currentAdjustments = database.prepare(`
    ...
    const activeAdjustments = currentAdjustments.map(adj => ({
      category: adj.profile_category,
    ...
    res.json({
      totalRejections: rejectionStats.totalRejections,
      activeAdjustments,
      topPatterns,
      recentLearnings: recentAdjustments.map(adj => ({
        timestamp: adj.created_at,
        category: adj.profile_category,
        adjustment: adj.new_weight - adj.old_weight,
        reason: adj.reason
      })),
      filterStats: filterStats.map(f => ({
        type: f.pattern_type,
        count: f.count
      }))
    });
